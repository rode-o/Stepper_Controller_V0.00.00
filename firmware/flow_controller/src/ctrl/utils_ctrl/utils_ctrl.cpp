...
uint16_t rateToTop(double uLmin)
{
    ...
    double fq  = (rpm / 60.0) * PPR;
    double top = SYSCLK / (PWM_CLKDIV * 2.0 * fq) - 1.0;
...
{
    constexpr double SYSCLK = 125'000'000.0;
    return SYSCLK / (PWM_CLKDIV * 2.0 * (top + 1));
}

uint16_t spsToTop(float sps)
{
    /* invert topToSps with same rounding guards */
    double rate = (sps * 60.0) / STEPS_PER_REV * VPR;
    return rateToTop(rate);
}
